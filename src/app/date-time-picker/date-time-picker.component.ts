import {
  ...
  ViewChild,
  AfterViewInit,
  DoCheck, forwardRef, OnChanges, SimpleChanges, Output, EventEmitter
} from "@angular/core";
import {
  NgbTimeStruct,
  NgbDateStruct,
...
export class DateTimePickerComponent
  ...
  ngOnInit(): void {
    ...
    if (this.dateString === null) {
      this.ptStr = "1700-01-01 00:00:00";
      ...
      this.ptTime = Object.assign(
        this.ptTime,
        {hour: dx.getHours(), minute: dx.getMinutes(), second: dx.getSeconds()});
  ...
  ngDoCheck() {
  //  console.log("ngDoCheck  ptTime >"+this.ptTime+"<");
  //  console.log("ngDoCheck  ptDate >"+this.ptDate+"<");
  //  console.log("ngDoCheck  ptStr >"+this.ptStr+"<");
  }
  ...
  setToday() {
    let dz = new Date();
    this.ptDate = { year: dz.getFullYear(), month: dz.getMonth()-1, day: dz.getDate()};
  ...
  onInputChange($event: any) {
    ...
    if (dt) {
      this.ptTime = {hour: dt.getHours(), minute: dt.getMinutes(), second: dt.getSeconds()};
      if (!this.enableTime) {
        this.ptTime = {hour: 0, minute: 0, second: 0};
      }
      this.ptDate = {year: dt.getFullYear(), month: dt.getMonth() + 1, day: dt.getDate()};
      if (this.required && this.ptDate.year === 1700 &&
        this.ptDate.month === 1 && this.ptDate.day === 1) {
        let date = new Date();
        this.ptTime = {hour: 0, minute: 0, second: 0};
        this.ptDate = {year: date.getFullYear(), month: date.getMonth()+1, day: date.getDate()};
      }
    } else if (value.trim() === "") {
      this.ptTime = {hour: 0, minute: 0, second: 0};
  ...
  setDateStringModel() {
    ...
    if (this.ptDate.year === 1700 && this.ptDate.month === 1 && this.ptDate.day === 1) {
      ...
      let dx = new Date(this.ptDate.year, this.ptDate.month-1, this.ptDate.day,
        this.ptTime.hour - this.diffTz, this.ptTime.minute, this.ptTime.second);
